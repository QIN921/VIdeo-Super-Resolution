WebFont.load({
    custom: {
        families: ['Hanalei Fill', 'Kirang Haerang', 'Pacifico', 'Permanent Marker', 'Shadows Into Light']
    }
});
...
function CaptchaMini(params = {}) {
    let middleParams = Object.assign({
        ...
        fontSize: 20,           //字体大小
        fontFamily: ['Hanalei Fill', 'Kirang Haerang', 'Pacifico', 'Permanent Marker', 'Shadows Into Light'],  //字体类型
...
CaptchaMini.prototype.line = function () {
    for (let i = 0; i < this.lineNum; i++) {
        /*随机获取线条的起始位置*/
        let x = this.getRandom(20, this.canvas.width - 20);
        let y = this.getRandom(0, this.canvas.height - 2);
        let endX = this.getRandom(20, this.canvas.width - 20);
        let endY = this.getRandom(0, this.canvas.height - 2);
...
CaptchaMini.prototype.draw = function (dom, callback = function () { }) {
    ...
    /*绘制画布*/
    r = 20, w = this.canvas.width, h = this.canvas.height;
    this.paint.lineJoin = 'round';  //交点设置成圆角
    this.paint.lineWidth = r;
    this.paint.strokeRect(r / 2, r / 2, w - r, h - r);
    this.paint.fillRect(r, r, w - r * 2, h - r * 2);
...
var code = '';
var fonts = ['Hanalei Fill', 'Kirang Haerang', 'Pacifico', 'Permanent Marker', 'Shadows Into Light'];
    ...
    fetch('https://v1.hitokoto.cn?c=i')
        ...
        .then(fuck => {
            var typed = new Typed('#typed', {
                strings: ["欢迎登录", fuck.hitokoto.toString()],//输出的文字
                typeSpeed: 50,//打字的速度
            });
        })
        .catch(console.error);
